...
mozartSchema.Dimensions(3).Length       = length(lat); % the lat dimension based on the geoschem output
mozartSchema.Dimensions(3).Unlimited    = false;
mozartSchema.Dimensions(4).Name         = 'lon';
mozartSchema.Dimensions(4).Length       = length(lon); % the lon dimension based on the geoschem output
...
latschema.Dimensions.Length               = length(lat); % the lev dimension based on the geoschem output
...
lonschema.Dimensions.Length               = length(lon); % the lev dimension based on the geoschem output
...
psschema.Dimensions(1).Length           = length(lon);
psschema.Dimensions(1).Unlimited        = false;
psschema.Dimensions(2).Name             = 'lat';
psschema.Dimensions(2).Length           = length(lat);
...
for ispec                                  = 1:varnum
    ...
    if (var_name_leng > 12)
        ...
        ncwrite(filename_out,spc_name_out,spc_conc);
    end
end
%%
